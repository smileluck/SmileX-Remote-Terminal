...
const DETECT_SCRIPT = [
  ...
  // nginx：从 nginx -V 解析 --conf-path / --prefix
  'np=$(command -v nginx 2>/dev/null); if [ -n "$np" ]; then nv=$(nginx -v 2>&1 | sed -E "s#.*/([0-9.]+)#\\1#"); nvv=$(nginx -V 2>&1); ncfg=$(echo "$nvv" | sed -nE "s/.*--conf-path=([^ ]+).*/\\1/p"); { [ -n "$ncfg" ] && [ -f "$ncfg" ]; } || ncfg=$(first_file /etc/nginx/nginx.conf); ndir=$(echo "$nvv" | sed -nE "s/.*--prefix=([^ ]+).*/\\1/p"); [ -n "$ndir" ] || ndir=$(dirname "$np"); emit nginx 1 "$nv" "$ndir" "$(svc_state nginx)" "$ncfg"; else emit nginx 0 "" "" "-" ""; fi',
  // openresty
