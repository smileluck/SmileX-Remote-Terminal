...
const DETECT_SCRIPT = [
  ...
  // python：优先级 pyenv > 系统包 > conda。.bashrc 里 conda init + 默认激活环境会
  // 把 login shell 的 python3 指向 conda env，遮蔽系统包 Python——故显式探测
  // /usr/bin/python3，conda 默认环境不影响系统包路径（conda 的 Python 由 Conda 卡片管理）。
  // 系统包路径取解释器所在 bin 目录（如 /usr/bin），非安装前缀
  ...
  // docker：路径指到数据根目录 /var/lib/docker（镜像/容器数据），第 7 字段附二进制目录
  'dp=$(command -v docker 2>/dev/null); if [ -n "$dp" ]; then dv=$(docker --version 2>/dev/null | sed -E "s/.*version ([0-9.]+).*/\\1/"); dbin=$(dirname "$(readlink -f "$dp" 2>/dev/null || echo "$dp")"); ddir="/var/lib/docker"; [ -d "$ddir" ] || ddir="$dbin"; dcfg=""; [ -f /etc/docker/daemon.json ] && dcfg="/etc/docker/daemon.json"; emit docker 1 "$dv" "$ddir" "$(svc_state docker)" "$dcfg" "$dbin"; else emit docker 0 "" "" "-" "" ""; fi',
].join('\n')
...
export async function switchVersion(sid: string, id: EnvId, version: string): Promise<void> {
    ...
    await runChecked(
      ...
      `bash -lc ${sq(
        [
          ...
          `if command -v pyenv >/dev/null 2>&1 && [ -n "$RPY" ] && [ "\${RPY#$HOME/.pyenv}" != "$RPY" ]; then pyenv install -s ${sq(v)} && pyenv global ${sq(v)}; elif command -v conda >/dev/null 2>&1; then conda install -y -n base python=${sq(v)}; else echo "系统包安装的 Python 不支持一键切换版本；如需版本管理，请先卸载系统包后通过面板安装（pyenv/conda 管理）"; exit 1; fi`,
        ].join('; '),
      )}`,
    )
    return
  ...
  if (id === 'go') {
    await runChecked(
      sid,
      'if [ -d /usr/local/go ]; then true; else echo "系统包安装的 Go 不支持一键切换版本；如需版本管理，请先卸载系统包后通过面板安装（官方包管理）"; exit 1; fi',
    )
